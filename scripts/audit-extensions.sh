...
# One pass proves:
#   - The five Extension packages build and typecheck
...
EXPECTED=("lsp" "question" "statusline" "subagents" "todo")
FAIL=0
...
note "filter targets exactly the five canonical packages"
...
[ "${#GOT[@]}" -eq 5 ] && ok "five Extension packages match the filter"
...
note "pnpm build:ext"
pnpm build:ext >/dev/null 2>&1 && ok "build:ext passes for all five" \
  || bad "build:ext failed"

note "pnpm typecheck:ext"
pnpm typecheck:ext >/dev/null 2>&1 && ok "typecheck:ext passes for all five" \
...
note "shipped settings.json points at the five current Extension packages"
...
if [ "${#MISSING[@]}" -gt 0 ]; then
  ...
  ok "settings.json references all five current Extension package paths"
fi
...
note "root package.json name/description match the five-package model"
...
note "README uses 'Extension package' for runnable packages, lists exactly five"
...
[ $FAIL -eq 0 ] && ok "README lists all five Extension packages"
...
heading "No broad unit-test infrastructure"
note "no *.test.ts / *.spec.ts under packages/"
HITS=$(find "$ROOT/packages" -name '*.test.ts' -o -name '*.spec.ts' 2>/dev/null || true)
if [ -n "$HITS" ]; then
  bad "test files found in packages/: $HITS"
else
  ok "no *.test.ts / *.spec.ts under packages/"
fi

note "no test framework dependency declared in any Extension package"
for name in "${EXPECTED[@]}"; do
  PJ="$ROOT/packages/$name/package.json"
...
[ $FAIL -eq 0 ] && ok "no test framework declared in any Extension package"
