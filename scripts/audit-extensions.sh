...
# One pass proves:
#   - The nine Extension packages build and typecheck
...
EXPECTED=("lsp" "question" "statusline" "subagents" "todo" "web-fetch" "web-search" "model-presets" "model-compaction")
FAIL=0
...
note "filter targets exactly the nine canonical packages"
...
[ "${#GOT[@]}" -eq 9 ] && ok "nine Extension packages match the filter"
...
note "pnpm build:ext"
pnpm build:ext >/dev/null 2>&1 && ok "build:ext passes for all nine" \
  || bad "build:ext failed"

note "pnpm typecheck:ext"
pnpm typecheck:ext >/dev/null 2>&1 && ok "typecheck:ext passes for all nine" \
...
note "shipped settings.json points at the nine current Extension packages"
...
if [ "${#MISSING[@]}" -gt 0 ]; then
  ...
  ok "settings.json references all nine current Extension package paths"
fi

heading "Root metadata tells the same story"
note "root package.json name/description match the nine-package model"
...
note "README uses 'Extension package' for runnable packages, lists exactly nine"
...
[ $FAIL -eq 0 ] && ok "README lists all nine Extension packages"
...
[ $FAIL -eq 0 ] && ok "all nine packages use the shared Vitest config for testing"
...
[ $FAIL -eq 0 ] && ok "all nine packages have __tests__/ with test files"
