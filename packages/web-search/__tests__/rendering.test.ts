...
function resultText(
	...
	return renderWebSearchResult(result, { isPartial, expanded }, plainTheme, undefined).render(80).join("\n");
}
...
describe("renderWebSearchCall", () => {
	...
	it("quotes each exact phrase", () => {
		expect(callText({ query: "patterns", exactPhrases: ["design patterns"] })).toContain('"design patterns"');
	});
